...
def _flag_true(name):
    return os.environ.get(name, "0").lower() in {"1", "true", "yes", "y", "on"}
...
def check_symmetry_config(
    ...
    # RFD3_ALLOW_EXACT_ASU_MOTIF_PATCH
    # For exact-translation symmetry (T<N>EXACT), allow one ASU motif to be
    # symmetrized internally by our custom translation frames. Generalized
    # to any T<N>EXACT id, matching the same generalization made in
    # frames.py's get_symmetry_frames_from_symmetry_id / _atom_array.
    # This affects only the overlay copy, not the installed RFD3 package.
    #
    # Checked flags, in order:
    #   RFD3_ALLOW_EXACT_ASU_MOTIF        (generalized, preferred)
    #   RFD3_ALLOW_<SYMMETRY_ID>_ASU_MOTIF  (e.g. RFD3_ALLOW_T2EXACT_ASU_MOTIF,
    #                                        kept for backward compatibility)
    _sid = str(sym_conf.id).lower()
    _is_exact_symmetry = (
        _sid.startswith("t") and _sid.endswith("exact") and _sid[1:-5].isdigit()
    )

    allow_t3exact_asu_motif = _is_exact_symmetry and (
        _flag_true("RFD3_ALLOW_EXACT_ASU_MOTIF")
        or _flag_true(f"RFD3_ALLOW_{_sid.upper()}_ASU_MOTIF")
    )
    ...
    if (
        ...
        and not allow_t3exact_asu_motif
    ):
        raise ValueError(
